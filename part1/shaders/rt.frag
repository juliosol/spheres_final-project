...
void main()
{
	...
	vec3 col;
    rt_material mat;
	vec3 pt,n; // Ppt is a point reference)
	vec4 ob;

	vec3 mask = vec3(1.0);
	vec3 color = vec3(0.0);
	vec3 ro = vec3(scene.camera_pos); // Ray origin
	vec3 rd = getRayDir(); // Rauy direction
	...
	{
		tm = calcInter(ro, rd, num, type); // tm could be point of intersection
		if(tm < maxDist)
		{
			pt = ro + rd*tm; /// Calculate point of intersection 
